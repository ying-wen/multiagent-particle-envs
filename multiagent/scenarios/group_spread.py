...
class Scenario(BaseScenario):
    ...
    def get_group_id(self, entity):
        return int(entity.name.strip().split(' ')[-1])
    ...
    def reset_world(self, world):
        ...
        #   landmarks
        for i, landmark in enumerate(world.landmarks):
            gid = self.get_group_id(landmark)
            landmark.color = np.array(self.group_colours[gid]) / 2. \
